...
**CRITICAL CONTEXT SEPARATOR RULE**: Context group separators (`--`) MUST ONLY be printed when context lines (`-A`, `-B`, or `-C`) are explicitly requested (`> 0`). When no context lines are requested (the default single-line matching mode), context separator lines (`--`) must NEVER be printed between non-adjacent matching lines.
...
- For files discovered via recursive directory traversal: Binary detection quits searching the file when a NUL byte is found. If matching content was encountered prior to or upon binary detection under standard search options (without `-a`/`--text` or `-q`/`--quiet`), ripgrep must emit a notice: `Binary file <PATH> matches`.
...
| Flag | Behavior |
|------|----------|
| `-m NUM` / `--max-count=NUM` | Stop searching a file immediately after NUM matching lines have been found and output for that file. |
...
When `--stats` is passed, ripgrep prints aggregate statistics after the search finishes.
