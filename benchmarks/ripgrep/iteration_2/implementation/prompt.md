...
3. **Skips binary files**: Files detected as binary (containing NUL bytes) are skipped by default during recursive search, with a brief notification.
...
rg [OPTIONS] --generate (man|complete-bash|complete-zsh|complete-fish|complete-powershell)
command | rg [OPTIONS] PATTERN
```

### Exit Codes

- `0`: Search operation found at least one match, OR an informational/utility mode (`--files`, `--type-list`, `--generate`, `--help`, `--version`) completed successfully without errors.
- `1`: Search operation found zero matches. (This exit code applies strictly to standard search operations when no matches are found).
- `2`: An error occurred (CLI argument parse error, invalid regex pattern, I/O error, etc.). Some non-fatal errors during search (e.g., permission denied on a single file during recursive traversal) are reported to stderr but do not cause immediate termination.

Broken pipe errors (e.g., `rg pattern | head`) should be treated as graceful termination with exit code 0.

### Version Information

Executing `rg --version` or `rg -V` must print `ripgrep <version>` on the first line of output, followed by any feature or target details.
...
| **Output formatting** | Consume search results and format them for display. Support standard grep-like format, summary/count format, vimgrep format, and JSON Lines format. |
...
- **Deferred validation**: Structure argument parsing so that "special modes" (like `--help`, `--version`, `--generate`, `--type-list`) can succeed even when the environment is invalid (e.g., missing current directory). This typically means separating raw flag parsing from the expensive validation/compilation step.
...
### 2.4 End-to-End Integration Requirements

**CRITICAL**: It is not sufficient to implement domain logic in separate library crates/modules — each component must be fully wired into the main execution pipeline. Specifically:

1. **CLI → Searcher → Printer wiring**: Every CLI flag that affects search behavior (`--max-count`, `--multiline`, `--binary`, `--null-data`, `--encoding`, etc.) must be propagated from the parsed CLI arguments to the appropriate searcher and printer builder methods. Every CLI flag that affects output format (`--no-filename`, `--with-filename`, `--heading`, `--color`, `--column`, `--byte-offset`, `--replace`, `--stats`, etc.) must be propagated to the printer builder.

2. **Directory walker → Ignore stack wiring**: The directory walker must actively evaluate the composite ignore stack (`.gitignore`, `.ignore`, `.rgignore`, glob overrides from `-g`/`--iglob`, and file type filters from `-t`/`-T`) against each file and directory entry during traversal. Entries that match ignore rules must be skipped before they are yielded for searching. The ignore stack, glob overrides, and file type filters are not advisory — they must be enforced during the walk.

3. **Statistics aggregation wiring**: When `--stats` is specified, the search pipeline must track aggregate counters (total matches, matched lines, files with matches, files searched, bytes searched, elapsed time) and print the statistics summary block after all files have been searched.

4. **Match limiting wiring**: When `-m N` / `--max-count=N` is given, the searcher must stop processing each file after `N` matching lines have been found and output. The search pipeline must pass this limit to the searcher.
...
| `-r REPLACEMENT` / `--replace=REPLACEMENT` | Replace matches with the given string. Supports capture group references by index (`$1`, `$2`) or name (`$name`, `${name}`). Literal `$` can be escaped as `$$`. |
| `-n` / `--line-number` | Show 1-based line numbers (default when output is a terminal). |
...
| `--vimgrep` | Print results in Vim errorformat: `PATH:LINE:COLUMN:LINE_CONTENT`. Filename and column number are always included regardless of `--no-filename` or heading flags. |
| `--files` | Print the paths of all files that would be searched without searching them. Paths must be formatted relative to the target search path directory argument (or relative to current working directory if no path argument was passed). Exits with code `0`. |
| `--stats` | Print aggregate search statistics (matched lines, matches, files searched, time spent) after search completes. |
...
### 3.2.1 Filename Display Defaults

The default behavior for showing/hiding the file path prefix on output lines is context-dependent:

- **Searching stdin**: Do NOT prefix the file path by default. Only prefix if `-H` / `--with-filename` is explicitly given.
- **Searching a single file (one positional path argument that is a file)**: Do NOT prefix the file path by default. Only prefix if `-H` / `--with-filename` is explicitly given.
- **Searching a directory or multiple paths**: DO prefix the file path by default. Suppress only if `--no-filename` is explicitly given.
- **`--no-filename`**: When given, ALWAYS suppress the file path prefix regardless of context, including in count mode (`-c`, `--count-matches`), files-with-matches (`-l`), and standard search output.
- **`-H` / `--with-filename`**: When given, ALWAYS show the file path prefix regardless of context.

This logic must be applied consistently across all output modes: standard search, count (`-c`), count-matches, files-with-matches, only-matching, replace, and any other mode that prints results.

### 3.2.2 Replacement with Capture Group Expansion

When `-r REPLACEMENT` / `--replace=REPLACEMENT` is provided:

1. For each matching line, the matched substring(s) must be replaced by the replacement text in the printed output.
2. If the replacement text contains capture group references (`$1`, `$2`, `${name}`, `$name`), these must be expanded to the actual text captured by the corresponding regex group.
3. Literal `$` is escaped as `$$`.
4. The replacement must use the regex engine's capture group functionality — the regex must be compiled with capture groups enabled when `--replace` is specified.
...
### 3.3.1 Context Separator Emission Rules

**CRITICAL**: Context group separators (`--`) must ONLY be printed when context lines (`-A`, `-B`, or `-C`) are explicitly requested with a value greater than 0. The rules are:

1. When `-A`, `-B`, or `-C` is specified with a value > 0, and there are non-contiguous match groups (i.e., non-adjacent matches separated by unprinted lines), print the context separator (`--` by default) between them.
2. When NO context flags are specified (the default single-line matching mode where before-context and after-context are both 0), context separator lines (`--`) must NEVER be printed, even between non-adjacent matching lines.
3. **Implementation requirement**: The searcher must only invoke context break events when the configured before-context or after-context count is greater than zero. Under default single-line matching (before-context = 0, after-context = 0), the searcher must never signal a context break, and the printer must never emit `--` separators.
...
| `--type-list` | List all supported file types and their globs, then exit with code `0`. |
...
| Flag | Behavior |
|------|----------|
| `--binary` | Search binary files. Don't print a warning for binary files found by directory traversal, but still suppress binary output lines. |
| `-a` / `--text` | Treat binary files as text. Print binary matches as-is. |
| `--no-binary` | Default behavior — skip binary files (print warning/notice when skipping matching binary content). |

Binary detection works by scanning for NUL bytes (`\x00`).
- For files discovered via recursive directory traversal: Binary detection quits searching the file when a NUL byte is found. If matching content was encountered prior to or upon binary detection under standard search options (without `-a`/`--text` or `-q`/`--quiet`), ripgrep must emit a notice to stderr: `Binary file <PATH> matches`.
- For files explicitly specified on the command line: Binary detection never causes the file to be skipped entirely, but matching binary data is suppressed and triggers the `Binary file <PATH> matches` message unless `-a`/`--text` is supplied.
...
| `--pcre2` / `-P` | Use PCRE2 regex engine. |
...
| `--null-data` | Use NUL (`\x00`) byte as the line record terminator instead of newline (`\n`) for file reading, line splitting, and pattern matching anchors (`^`, `$`). |

### 3.7 Search Behavior

| Flag | Behavior |
|------|----------|
| `-m NUM` / `--max-count=NUM` | Stop searching a file immediately after NUM matching lines have been found and output for that file. This limit must be enforced during search execution, not just parsed. |
...
| `--sort=CRITERIA` | Sort search results in ascending order by path, modified time, accessed time, or created time. Sorting forces single-threaded mode. |
| `--sortr=CRITERIA` | Sort search results in descending (reverse) order by path, modified time, accessed time, or created time. Sorting forces single-threaded mode. |
...
ripgrep can generate shell completions and a man page from its CLI flag definitions:

| Flag | Output |
|------|--------|
| `--generate=man` | Generate man page in roff format to stdout. |
| `--generate=complete-bash` | Generate Bash completions to stdout. |
| `--generate=complete-zsh` | Generate Zsh completions to stdout. |
| `--generate=complete-fish` | Generate Fish completions to stdout. |
| `--generate=complete-powershell` | Generate PowerShell completions to stdout. |

When `--generate=<MODE>` is executed, ripgrep must emit the requested generated content to stdout and exit immediately with exit code `0`.

---

## 4. Ignore File Precedence

Ignore rules are applied in a strict precedence order. Rules in higher-precedence files override rules in lower-precedence files, regardless of directory depth:

1. **`--glob` / `--iglob`** flags and explicit CLI paths (highest precedence)
2. **Explicit CLI ignore flags** (`--no-ignore`, etc.)
3. **`.rgignore`** in current directory and ancestor directories
4. **`.ignore`** in current directory and ancestor directories
5. **`.gitignore`** in current directory and ancestor directories
6. **Global git ignore** (from `core.excludesFile` in git config)

Each ignore file follows gitignore pattern syntax. Within a single ignore file, later rules override earlier rules. Rules in a child directory override rules in a parent directory within the same file tier. Crucially, a rule in `.rgignore` takes precedence over any conflicting rule in `.gitignore`.

A `.gitignore` or `.rgignore` rule can be negated with `!` prefix to un-ignore paths.

---

## 5. File Type System

ripgrep has a built-in registry of file type definitions mapping type names to glob patterns. The registry must be comprehensive, covering at minimum the following commonly-used types:

| Type Name | Glob Patterns |
|-----------|--------------|
| `agda` | `*.agda`, `*.lagda` |
| `awk` | `*.awk` |
| `c` | `*.c`, `*.h`, `*.H`, `*.cats` |
...
- List all types: `rg --type-list` (prints definitions and exits with status `0`).
...
- **`summary`**: Emitted at the very end when `--stats` is specified. Contains: `elapsed_total`, `stats` (aggregate search statistics).

The `text` field uses lossy UTF-8. The `bytes` field is base64-encoded raw bytes.

### 6.1 Match Object Schema

Each `"match"` JSON object must contain the following fields inside a `"data"` wrapper:

```json
{
  "type": "match",
  "data": {
    "path": {"text": "relative/path.txt"},
    "lines": {"text": "the full matching line\n"},
    "line_number": 42,
...
The `submatches` field is **mandatory** on every `"match"` object. It is an array where each entry records a distinct match on the line, with:
- `match.text`: the text of the matched substring
- `start`: the 0-based byte offset of the match start within the line
- `end`: the 0-based byte offset of the match end within the line
...
## 14. Stdin Detection and Default Search Path

When no positional PATH arguments are provided on the command line:

1. **If standard input is a pipe or redirect (not a TTY)**: Read from standard input and search it. Do NOT fall back to searching the current directory.
2. **If standard input is a TTY (interactive terminal)**: Search the current directory (`.`) recursively.

This check must use a TTY detection mechanism (e.g., `std::io::IsTerminal` or `atty`) on the process's stdin file descriptor. The key principle: piped input (`echo "foo" | rg "foo"`) must always search stdin, never the filesystem.
...
## 18. Key Design Decisions to Preserve
...
## 19. Multiline Search Mode

When `-U` / `--multiline` is enabled:

1. The searcher must NOT operate strictly line-by-line. Instead, it must read file content into a buffer (or use memory-mapped I/O) and execute regex matches against the full multi-line buffer.
2. Patterns containing `\n` or constructs that span line boundaries (e.g., `foo\nbar`) must be able to match across lines.
3. Line numbers in multiline mode are computed from byte offsets within the buffer, not from a line iterator.
4. `--multiline-dotall` makes `.` also match `\n` in multiline mode.
5. In the absence of `--multiline`, the searcher operates line-by-line as usual.

---

## 20. Null-Data Mode and Binary Detection Interaction

When `--null-data` is enabled:

1. The line terminator is NUL (`\x00`) instead of newline (`\n`).
2. Binary detection MUST be disabled, because NUL bytes are the valid line separator and must not trigger binary abort.
3. Pattern matching anchors (`^`, `$`) match at NUL boundaries.

When `--null-data` is NOT enabled (normal mode):
1. Binary detection scans for NUL bytes. If a NUL byte is found, the file is considered binary.
2. If a binary file matches (content matched before binary detection triggered), ripgrep must emit `Binary file <PATH> matches` to stderr (unless suppressed by `-q`, `--quiet`, or summary modes like `-c`).

---

## 21. Directory Traversal and Sorting

### 21.1 Ignore Stack Enforcement

The directory walker must enforce the full ignore stack during traversal:

1. Before yielding or descending into any directory entry, check it against all active ignore rules in precedence order (see Section 4).
2. Before yielding any file for searching, check it against glob overrides (`-g`/`--iglob`) and file type filters (`-t`/`-T`).
3. Entries that match any active ignore/exclude rule must be skipped entirely — they must not be yielded to the search pipeline.
4. This filtering must happen inside the directory walker, not as a post-filter after the walker has yielded all entries.

### 21.2 Path Sorting

When `--sort=CRITERIA` or `--sortr=CRITERIA` is specified:

1. The directory walker must collect entries and sort them according to the specified criteria before yielding them.
2. For `--sort path`: entries are sorted in ascending lexicographic order by their relative path.
3. For `--sortr path`: entries are sorted in descending lexicographic order by their relative path.
4. For time-based sorting (`modified`, `accessed`, `created`): entries are sorted by the respective file metadata timestamp.
5. Sorting forces single-threaded mode to preserve the sorted order in output.
6. Both `--files` listing and search result output must respect the sort order.

---

## 22. Statistics Output

When `--stats` is passed, ripgrep must track aggregate counters throughout the search and print a statistics summary block after all search output has been printed.

In text mode, it outputs:
```
<N> matches
<N> matched lines
...
In JSON mode (`--json --stats`), a `summary` JSON message is emitted containing `stats` and `elapsed_total` fields.

The statistics must reflect actual search execution data, not be omitted or left as zeros.

---

## 23. Build & Test Expectations
